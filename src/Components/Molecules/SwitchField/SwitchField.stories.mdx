...
export const Template = (args) => {
  ...
  return <SwitchField {...args} inputValue={val} onChange={setVal} />;
};

<Canvas withSource='open'>
  <Story name='SwitchField' args={{ label: 'Switch field', inputValue: options }}>
    {Template.bind({})}
  </Story>
</Canvas>
...
<Canvas>
  <Template label='Switch field in error' errorMessage='This text is on error' inputValue={options} />
</Canvas>
...
<Canvas>
  <Template disabled={true} helperText='Helper text' label='Switch field disabled' inputValue={options} />
</Canvas>

### Inline

<Canvas>
  <Template label='Switch field in line' inline={true} inputValue={[options[0], options[1]]} />
</Canvas>
